...
class ANA2B(tf.keras.Model):
    def __init__(self, cutoff=10.0, n_units=128, node_size=64, n_layers=2, n_steps=1, activation=tf.nn.swish, n_gaussians=5, debug=False):
        ...
        self.S_ind = ff_module(self.n_units, self.n_layers, activation=self.activation, output_size=1, final_activation=tf.nn.softplus)
        self.K = ff_module(self.n_units, self.n_layers, activation=self.activation, output_size=5, final_activation=tf.nn.softplus)
    ...
    def __call__(self, graph_1, graph_2, coords_1, coords_2, distance_matrices, multipoles, induced_mus, batch_size):          
        S_features, S_features_ind, K_features, R1, R2, switch_function, indices_b =\
                     self._build_features(coords_1, coords_2, distance_matrices, graph_1, graph_2, multipoles, induced_mus)
        V_ex, V_at = self._calculate_energy_terms(S_features, S_features_ind, K_features, R1, R2, switch_function)        
        return tf.math.unsorted_segment_sum(V_ex - V_at, indices_b, num_segments=batch_size)[:, 0]

    @tf.function(experimental_relax_shapes=True)
    def _calculate_energy_terms(self, S_features, S_features_ind, K_features, R1, R2, switch_function):
        S2_static, S2_at = tf.split(self.S_static(S_features) * switch_function, 2, axis=-1)
        S2_ind = self.S_ind(S_features_ind) * switch_function
        K1_static, K1_ind, K2_static, K2_ind, K_at = tf.split(self.K(K_features), 5, axis=-1)
    ...
    @tf.function(experimental_relax_shapes=True)
    def _build_features(self, coords_1, coords_2, distance_matrices, graph_1, graph_2, multipoles, induced_mus):
        ...
        dipoles_ind_1, dipoles_ind_2 = induced_mus
        ...
        dipos_ind_1, dipos_ind_2 = tf.gather_nd(dipoles_ind_1, indices_1), tf.gather_nd(dipoles_ind_2, indices_2)
        ...
        G_features_ind = G_matrices_ind_sym(monos_1, monos_2, dipos_1 + dipos_ind_1, dipos_2 + dipos_ind_2, quads_1, quads_2, Rx1, Rx2)
        ...
        return S_features, S_features_ind, K_features, R1, R2, switch_function, indices_b
...
@tf.function(experimental_relax_shapes=True)
def G_matrices_ind_sym(monos_1, monos_2, dipos_1, dipos_2, quads_1, quads_2, Rx1, Rx2):
    D1_Rx1, D2_Rx1 = S(dipos_1, Rx1), S(dipos_2, Rx1)
    Q1_Rx1, Q2_Rx1 = tf.einsum('ijk, ik -> ij', quads_1, Rx1), tf.einsum('ijk, ik -> ij', quads_2, Rx1)
    Q1_Rx2, Q2_Rx2 = A(tf.einsum('ijk, ijk -> i', quads_1, Rx2)),  A(tf.einsum('ijk, ijk -> i', quads_2, Rx2))
    dipo_mono = D1_Rx1 * monos_2 
    ...
    G3 = tf.concat((-4 * S(Q1_Rx1, Q2_Rx1), 
                    -Q1_Rx2 * D2_Rx1 + Q2_Rx2 * D1_Rx1), axis=-1) # sym
    return tf.concat((G1, G2, G3), axis=-1)
